...
public class DiffMatchPatch {
	...
	public String diff_toDelta(LinkedList<Diff> diffs) {
		...
		if (delta.length() != 0) {
			...
			delta = Utils.unescapeForEncodeUriCompatability(delta);
		}
		return delta;
	}
	 ...
	 */
	private Map<Character, Integer> match_alphabet(String pattern) {
	 ...
	 *            Source text.
	 */
	private void patch_addContext(Patch patch, String text) {
		if (text.length() == 0) {
			return;
		}
	 ...
	 *             If invalid input.
	 */
	public LinkedList<Patch> patch_fromText(String textline) throws IllegalArgumentException {
		LinkedList<Patch> patches = new LinkedList<Patch>();
