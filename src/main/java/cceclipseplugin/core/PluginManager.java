...
//				ResourcesPlugin.getWorkspace().getRoot().getLocation().toString());
		...
		System.out.println("PROJECT_METADATA: " + metadataManager.getProjectMetadata(
				ResourcesPlugin.getWorkspace().getRoot().getLocation().toString()+"/"));
	}
	...
	private void websocketLogin() throws ConnectException, InterruptedException {
		...
		IRequestData data1 = new UserRegisterRequest(StringConstants.PREFERENCES_USERNAME,
				StringConstants.PREFERENCES_FIRSTNAME, StringConstants.PREFERENCES_LASTNAME,
				StringConstants.PREFERENCES_EMAIL, StringConstants.PREFERENCES_PASSWORD);
		Request req1 = new Request("User", "Register", data1, resp -> {
			IRequestData data2 = new UserLoginRequest(StringConstants.PREFERENCES_USERNAME,
					StringConstants.PREFERENCES_PASSWORD);
			Request req2 = new Request("User", "Login", data2, response -> {
